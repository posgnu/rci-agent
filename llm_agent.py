...
class LLMAgent:
    ...
    def get_plan_step(self):
        num_step = self.current_plan.count("\n")
    ...
    def get_response(self, pt):
        ...
        while True:
            try:
                if self.llm == "chatgpt" or self.llm == "gpt4" or self.llm == "chatgpt-16":
    ...
    def clean_xpath(self, xpath: str):
        if xpath[0] == '"':
            xpath = xpath[1:]
        if xpath[-1] == '"':
            xpath = xpath[:-1]
